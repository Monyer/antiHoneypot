  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    if (!GLOBAL.sameIds[ids[i]].includes(domain)) {
      ...
      if (CONF.sameIdAlert) {
        ...
        sendNotice(sender.tab.id, sender.url, "【提示】跟踪标识", msg);
      }
    }
  }
}
...
function onMessageCallback(request, sender, sendResponse) {
  ...
  if (request.msgType == "openDatabase" && CONF.openDatabaseAlert) {
    ...
    sendNotice(sender.tab.id, sender.url, "【提示】打中openDatabase规则", msg);
  }

  //getClipboard
  if (request.msgType == "getClipboard" && CONF.getClipboardAlert) {
    let msg = "这个网页调用了剪切板粘贴取值函数！";
    sendNotice(sender.tab.id, sender.url, "【提示】打中getClipboard规则", msg);
  }
  ...
  if (request.msgType == "requestFileSystem" && CONF.requestFileSystemAlert) {
    ...
    sendNotice(sender.tab.id, sender.url, "【提示】打中requestFileSystem规则", msg);
  }
  ...
  if (request.msgType == "honeypotAlert" && !GLOBAL.honeypotDomains.includes(urlDomain)) {
    ...
    sendNotice(sender.tab.id, sender.url, "【提示】蜜罐前端检测", msg);
  }
  ...
  if (request.msgType == "fingerprint2") {
    ...
    sendNotice(sender.tab.id, sender.url, "【提示】打中fingerPrintJs规则", msg);
  }
  ...
  if (request.msgType == "fingerprint" && GLOBAL.fingerPrints[sender.tab.id] !== true) {
    ...
    if (before_len < 4 && after_len >= 4 && CONF.fingerPrintAlert) {
      ...
      sendNotice(sender.tab.id, sender.url, "【提示】打中fingerprint规则", msg);
    }
  }
  ...
  if (request.msgType == "isObfuscator" && CONF.obfuscatorAlert) {
    ...
    sendNotice(sender.tab.id, sender.url, "【提示】打中Obfuscator规则", msg);
  }

  //判断是否是Beef
  if (request.msgType == "beefAlert") {
    let msg = "这个页面中含有BEEF跟踪脚本";
    sendNotice(sender.tab.id, sender.url, "【提示】打中beef规则", msg);
  }

  if (request.msgType == "opencanary") {
    let msg = "这个页面疑似OpenCanary蜜罐，请小心！";
    sendNotice(sender.tab.id, sender.url, "【提示】打中OpenCanary规则", msg);
  }

  //判断是否hfish
  if (request.msgType == "hfish") {
    let msg = "这个页面疑似HFish蜜罐，请小心！";
    sendNotice(sender.tab.id, sender.url, "【提示】打中HFish规则："+request.msgData, msg);
  }
